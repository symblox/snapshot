...
const actions = {
  ...
  getGovernorParams: async ({ commit }, space) => {
    ...
    try {
      const auth = getInstance();
      const governorContract = await getContract(
        space.governor,
        'Governor',
        auth.web3
      );
      ...
      const proposals: any = await Promise.all(
        proposalCreatedLogs.map(async log => {
          ...
          if (curBlockNumber > endBlock) {
            ...
            endTimestamp =
              curTimestamp + (endBlock - curBlockNumber) * secondsPerBlock;
          }
  ...
  getProposal: async ({ commit }, payload) => {
    ...
    try {
      ...
      if (blockNumber > proposal.startBlock) {
        startTimestamp = await getBlockTimestamp(
          provider,
          parseFloat(proposal.startBlock)
        );
      } else {
        startTimestamp =
      ...
      if (blockNumber > proposal.endBlock) {
        endTimestamp = await getBlockTimestamp(
          provider,
          parseFloat(proposal.endBlock)
        );
      } else {
        endTimestamp =
          curTimestamp + (proposal.endBlock - blockNumber) * secondsPerBlock;
      }
